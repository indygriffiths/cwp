...
CWP environments run PHP 5.6 by default (see Debian "Jessie" [packages](https://packages.debian.org/jessie/)). They can be upgraded to run PHP 7.3 via a request to the helpdesk. SilverStripe sites running Framework/CMS version 3.6 and up support PHP 7.
